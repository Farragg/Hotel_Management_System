...
@section('content')
	...
	<div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
	  <div class="carousel-inner">
	    ...
	    <div class="carousel-item @if($index==0) active @endif">
	      <img src="{{asset('images/banners/'.$banner->banner_src)}}" class="d-block w-100" alt="{{$banner->alt_text}}">
	    </div>
	    @endforeach
	  </div>
	...
	<div class="container my-4">
		...
		<div class="row my-4">
			<div class="col-md-3">
				<a href="{{url('service/'.$service->id)}}"><img class="img-thumbnail" style="width:100%;" src="{{ $service->image_path}}" alt=""></a>
			</div>
			<div class="col-md-8">
				<h3>{{$service->title}}</h3>
				...
				<div class="card">
					...
					<div class="card-body">
						@foreach($rtype->roomtypeimgs as $index => $img)
                        	<a href="{{asset('images/roomtypes/'.$img->img_src)}}" data-lightbox="rgallery{{$rtype->id}}">
                        		@if($index > 0)
                        		<img class="img-fluid hide" src="{{asset('images/roomtypes/'.$img->img_src)}}" />
                        		@else
                        		<img class="img-fluid" src="{{asset('images/roomtypes/'.$img->img_src)}}" />
                        		@endif
                        	</a>
                        </td>
                        @endforeach
